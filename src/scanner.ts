...
class Scanner {
  ...
  private scanToken(): void {
    ...
    switch (c) {
      ...
      case "!": {
        this.addToken(
          this.matchOperator("=") ? TokenType.BANG_EQUAL : TokenType.BANG
        )
        break
      }
      case "=": {
        this.addToken(
          this.matchOperator("=") ? TokenType.EQUAL_EQUAL : TokenType.EQUAL
        )
        break
      }
      case "<": {
        this.addToken(
          this.matchOperator("=") ? TokenType.LESS_EQUAL : TokenType.LESS
        )
        break
      }
      case ">": {
        this.addToken(
          this.matchOperator("=") ? TokenType.GREATER_EQUAL : TokenType.GREATER
        )
        break
      }
      ...
      case "/": {
        if (this.matchOperator("/")) {
          ...
          if (nestCount > 0 && this.isAtEnd()) {
            Lox.err(
              this.line,
              "Unexpected End of file => Unclosed block Comment"
            )
          }
          this.advance()
        } else {
